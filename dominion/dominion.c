...
int oracle(struct gameState *G, int seed, int cardToTest){
    //rand needs to be seeded before this function is called
    ...
    int turns = 0;
    int eligibleAfterTurnNumber = rand() % 20;
    int money = 0;
    int smithyPos = -1;
    int adventurerPos = -1;
    int greatHallPos = -1;
    ...
    while (!isGameOver(G)) {
        ...
        if(adventurerPos != -1 && cardToTest == adventurer){
            printf("Testing random adventurer card\n");
            return adventurerPos;
        }
        if(smithyPos != -1 && cardToTest == smithy){
            printf("Testing random smithy card\n");
            return smithyPos;
        }
        if(greatHallPos != -1 && cardToTest == great_hall){
